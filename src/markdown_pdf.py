"""Render any markdown document to a branded PDF (reportlab, no new dependency).

Built for deep-prospecting research packs, but the repo's real payload is the
hand-written knowledge bases -- the curative-title playbooks, the NEPQ playbook
and rubric, the novation playbook, the open-records request template -- all of
which a partner, rep or county clerk would rather receive as a PDF than a .md.

Complements report_generator.py rather than duplicating it: that one renders a
NoticeData dataclass into a fixed per-record layout and accepts no markdown at
all; this one takes arbitrary markdown.
...
CLI:
    python src/markdown_pdf.py <any.md> [out.pdf]
"""
...
FOOTER_NOTE = "SiftStack - grounded in API / county-record / public-record sources; confidential"
...
def _inline(text: str) -> str:
    ...
    # Italic must not span whitespace: the original `\*(.+?)\*` let a single
    # stray asterisk (common in legal descriptions and redacted addresses)
    # swallow everything up to the next one, italicising whole sentences.
    ...
    while i < len(lines):
        ...
        if stripped.startswith("# "):
            ...
            story.append(Paragraph(_inline(body), BULLET2 if indent >= 2 else BULLET,
                                   bulletText="•"))
        # Ordered lists: without this, numbered SOP/playbook steps silently
        # collapsed into plain paragraphs and lost their sequence — the exact
        # structure a rep-facing procedure depends on.
...
def generate_pdf(md_path, pdf_path=None) -> str:
    ...
    doc = SimpleDocTemplate(
        ...
        title=md_path.stem, author="SiftStack",
    )
