...
SYSTEM_PROMPT = (
    "You extract structured data from Texas legal notices. "
    "Return ONLY valid JSON with no markdown formatting, no code fences, no explanation."
)

USER_PROMPT_TEMPLATE = """\
Extract the following fields from this {notice_type} legal notice published in {county} County, Texas.
...
Extract the following fields from this probate "Notice to Creditors" published in {county} County, Texas.
...
- "owner_state": the state of the PR's mailing address (usually "TX")
...
Extract the following fields from this eviction notice / detainer warrant from {county} County, Texas.
...
Extract the following fields from this code violation notice from {county} County, Texas.
...
Extract the following fields from this divorce filing / complaint from {county} County, Texas.
...
Classify this legal document from {county} County, Texas into one of these categories:
